"""
============================================================
FireGuardian Baseline V1 - 训练入口 (Train Entry)
============================================================
按照《9.FireGuardian Baseline V1 中等规模训练与精度评估任务.docx》执行：
- 通过 M2 YOLOTrainer 训练（不绕过，不使用 CLI yolo train）
- weights=yolo11n.pt（不使用 best.pt / smoke_v1_best.pt 作为起点）
- epochs=30, batch=16, imgsz=640, device=0, workers=4, patience=10, seed=42
- project=runs/baseline_train, name=baseline_v1
- best 输出隔离: models/baseline_v1_best.pt（不覆盖 models/best.pt）
- 逐轮指标 -> tools/baseline_v1_metrics.jsonl
- 训练后解析 results.csv（权威逐轮损失/LR）-> tools/baseline_v1_training_summary.json
- 复制 last.pt -> models/baseline_v1_last.pt，并做 SHA-256 校验

用法:
  python tools/baseline_v1_train_entry.py
============================================================
"""
...
SEED = 42
METRICS_PATH = _ROOT / "tools" / "baseline_v1_metrics.jsonl"
SUMMARY_PATH = _ROOT / "tools" / "baseline_v1_training_summary.json"
BEST_TARGET = _ROOT / "models" / "baseline_v1_best.pt"
LAST_TARGET = _ROOT / "models" / "baseline_v1_last.pt"
RESULT_DIR = _ROOT / "runs" / "baseline_train" / "baseline_v1"
...
def parse_results_csv(csv_path):
    """解析 Ultralytics results.csv，返回逐轮权威指标"""
...
def main() -> int:
    ...
    def on_epoch(info: dict):
        ...
        if info.get("kind") == "epoch":
            print("[记录] epoch %s | P %.4f | R %.4f | mAP50 %.4f | mAP50-95 %.4f | lr %s | gpu %.0fMB | elapsed %.0fs" % (
                ...
                info.get("mAP50-95", 0) or 0, info.get("lr"), info.get("gpu_mem_mb", 0) or 0,
                rec["elapsed_s"]))

    trainer = YOLOTrainer(
        weights="yolo11n.pt",
        dataset_yaml="datasets/fire_smoke_baseline_v1.yaml",
        project="runs/baseline_train",
        name="baseline_v1",
    )
    print("数据集 yaml:", trainer.dataset_yaml)
    print("训练参数: epochs=30 batch=16 imgsz=640 device=0 workers=4 patience=10")
    ...
    # 合并：以 results.csv 为准补充损失/LR，回调补充显存
    merged = []
    ...
    early_stopped = bool(merged) and merged[-1]["epoch"] < 30

    # 复制 last.pt 并 SHA-256 校验
    ...
    print("[保存] 训练汇总:", SUMMARY_PATH)
    if best:
        print("[最佳] epoch=%s mAP50=%.4f mAP50-95=%.4f P=%.4f R=%.4f" % (
            best["epoch"], best["mAP50"], best["mAP50-95"], best["precision"], best["recall"]))
    print("[完成] 总耗时: %.1fs (%.1f min)" % (total_seconds, total_seconds / 60))
    return 0
