...
def log_event(event, event_type: str = "event_updated",
              ...
              level: str = "INFO") -> bool:
    """
    将事件以统一字段写入 events.jsonl（改进说明 2）。
    ...
    if ok:
        danger = details.get("danger_level")
        if danger in (None, ""):
            md = details.get("metadata")
            if isinstance(md, dict):
                danger = md.get("danger_level", "N/A")
        if danger in (None, ""):
            danger = "N/A"
        get_logger(module).info(f"{event_type} | {event_id} | 危险等级 {danger}")
    return ok


def log_event_confirmed(event, module: str = "event_aggregator") -> bool:
    return log_event(event, "event_confirmed", module)


def log_event_updated(event, module: str = "event_aggregator") -> bool:
    return log_event(event, "event_updated", module)


def log_event_ended(event, module: str = "event_aggregator") -> bool:
    return log_event(event, "event_ended", module)


def log_event_discarded(event, module: str = "event_aggregator") -> bool:
    return log_event(event, "event_discarded", module)


def log_decision(event, decision, module: str = "fire_decision_agent") -> bool:
    ...
    return log_event(event, "decision_made", module, details=details)
...
def main():
    ...
    log_event_confirmed(event)
    log_event_updated(event)
    ...
    log_event_ended(event)
    flush()

    events_text = _EVENTS_FILE.read_text(encoding="utf-8")
    lines = [_json.loads(l) for l in events_text.splitlines() if l.strip()]
    assert len(lines) >= 4, f"事件日志行数不足: {len(lines)}"
    rec = lines[0]
    for key in ("time", "level", "module", "event_type", "event_id", "frame_id", "details"):
