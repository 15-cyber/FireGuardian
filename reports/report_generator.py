...
功能（按改进说明实现）：
  ...
  - 报告内容：基本信息 / 检测统计 / 事件时间线 / Agent 分析 / 图片证据 / 免责声明
...
_DISCLAIMER_DEFAULT = "本系统结果仅用于辅助监测与演示，不替代专业消防判断和企业应急预案。"


def _level_str(level) -> str:
    """危险等级枚举 → 字符串（medium/high），避免输出 DangerLevel.HIGH"""
...
class ReportGenerator:
    ...
    def generate(self, data: EventReportData) -> List[Path]:
        ...
        json_path = self._write_event_json(event_dir, data)
        if json_path:
            output.append(json_path)

        # 2. 复制截图到 images/
        image_map = self._collect_images(data, images_dir)

        # 3. report.md
        md_path = self._write_markdown(event_dir, data, image_map)
        if md_path:
            output.append(md_path)

        # 4. report.pdf（无中文字体时明确提示并跳过）
        if "pdf" in self.formats:
            pdf_path = self._write_pdf(event_dir, data, image_map)
            if pdf_path:
    ...
    def _collect_images(self, data: EventReportData, images_dir: Path) -> Dict[str, str]:
        """把截图复制到报告 images/，返回 reason -> 相对路径（优先标注图）"""
    ...
    def _write_event_json(self, event_dir: Path, data: EventReportData) -> Optional[Path]:
    ...
    def _write_markdown(self, event_dir: Path, data: EventReportData,
                        image_map: Dict[str, str]) -> Optional[Path]:
        """report.md：人类可读报告"""
        evt = data.event
        ...
        danger = evt.metadata.get("danger_level", "N/A")
    ...
    def _write_pdf(self, event_dir: Path, data: EventReportData,
                   image_map: Dict[str, str]) -> Optional[Path]:
        """report.pdf：正式文档（需要中文字体；任何失败都不崩溃，降级为 md+json）"""
        ...
        try:
            ...
            evt = data.event
            story = [
                Paragraph("火灾事件分析报告", h1),
                Paragraph(f"事件编号：{evt.event_id}", body),
                Spacer(1, 6),
                Paragraph("1. 基本信息", h2),
            ]
            ...
            info_rows = [
                ...
                ["结束时间", f"{evt.end_timestamp:.1f}s" if evt.end_timestamp else "进行中"],
                ["持续时长", f"{evt.duration:.1f}s"],
                ["总处理帧数", str(evt.total_frames)],
                ["阳性帧数", str(evt.positive_frames)],
            ]
            t1 = Table(info_rows, colWidths=[60 * mm, 110 * mm])
            ...
            story.append(Paragraph("2. 检测统计", h2))
            trend = evt.growth_trend.value if hasattr(evt.growth_trend, "value") else str(evt.growth_trend)
            stats_rows = [
                ...
                ["增长趋势", trend],
                ["最高危险等级", str(evt.metadata.get("danger_level", "N/A"))],
            ]
            t2 = Table(stats_rows, colWidths=[60 * mm, 110 * mm])
            ...
            story.append(Paragraph("3. 事件时间线", h2))
            for line in self._build_timeline(data):
                story.append(Paragraph(line, body))

            story.append(Paragraph("4. Agent 分析", h2))
            if not data.decisions:
            ...
            if image_map:
                story.append(Paragraph("5. 图片证据", h2))
                for reason in ("confirmed", "peak", "danger_level_upgraded", "final"):
                    rel = image_map.get(reason)
                    if not rel:
                        continue
                    ...
                    story.append(Paragraph(f"{reason}", h2))
                    story.append(Image(BytesIO(data_bytes), width=w * scale, height=h * scale))

            story.append(Paragraph("6. 免责声明", h2))
...
def main():
    ...
    for must in ("事件编号", "检测统计", "事件时间线", "Agent 分析", "图片证据", "免责声明"):
